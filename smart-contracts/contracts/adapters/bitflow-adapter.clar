;; title: bitflow-adapter
;; version: 1.0.0
;; summary: Adapter for Bitflow sBTC/STX LP yield
;; description: Deposits into Bitflow sBTC/STX liquidity pools for LP yield.
...
(define-public (adapter-deposit (amount uint))
  (begin
    ...
    (print {event: "bitflow-deposit", amount: amount, new-balance: (var-get deposited-balance)})
    (ok amount)
  )
)
...
(define-public (adapter-withdraw (amount uint))
  (begin
    ...
    (var-set deposited-balance (- (var-get deposited-balance) amount))
    (print {event: "bitflow-withdraw", amount: amount, new-balance: (var-get deposited-balance)})
    (ok amount)
  )
)
...
(define-public (harvest)
  (let (
    ...
    (print {event: "bitflow-harvest", yield: yield-amount})
    (ok yield-amount)
  )
)
