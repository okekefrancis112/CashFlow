;; title: stackingdao-adapter
;; version: 1.0.0
;; summary: Adapter for StackingDAO liquid stacking
;; description: Deposits STX into StackingDAO for stSTX liquid stacking yield.
...
(define-public (adapter-deposit (amount uint))
  (begin
    ...
    (print {event: "stackingdao-deposit", amount: amount, new-balance: (var-get deposited-balance)})
    (ok amount)
  )
)
...
(define-public (adapter-withdraw (amount uint))
  (begin
    ...
    (var-set deposited-balance (- (var-get deposited-balance) amount))
    (print {event: "stackingdao-withdraw", amount: amount, new-balance: (var-get deposited-balance)})
    (ok amount)
  )
)
...
(define-public (harvest)
  (let (
    ...
    (print {event: "stackingdao-harvest", yield: yield-amount})
    (ok yield-amount)
  )
)
